...
void loop() {
  ...
  if (elapsedTime > mdt && myController.isConnected() &&  myController.joystick.getButtonState()==carval){
  Serial.printf("%d %.1f %.1f\n", myController.joystick.getButtonState(),
  myController.joystick.getX(), myController.joystick.getY());
  elapsedTime = 0;
  nowTime = millis();
  }
  else if (elapsedTime > mdt && myController.isConnected() &&  myController.joystick.getButtonState()==polarval ){
    Serial.printf("%d %.1f %.1f\n",myController.joystick.getButtonState(),
